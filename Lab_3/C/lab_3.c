...
int rselect(int *list, int low, int high, int k){
	...
	if(k==rank)
		...
		return rselect(list, low, pivot_index+1, k);
	else
		return rselect(list, pivot_index-1, high, k-rank-1);
}
...
int rselect_bad(int *list, int low, int high, int k){
	if(low == high)
		return list[low];
	int pivot_index = (rand()%(high - low + 1)) + low;
	pivot_index = partition_bad(list, low, high, pivot_index);
	int rank = pivot_index-low;
	if(k==rank)
		return list[pivot_index];
	else if(k < rank)
		return rselect_bad(list, low, pivot_index+1, k);
	else
		return rselect_bad(list, pivot_index-1, high, k-rank-1);
}
